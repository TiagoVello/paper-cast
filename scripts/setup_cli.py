...
def uninstall_command(args: argparse.Namespace) -> int:
    ...
    say()
    say("Left behind, on purpose — none of it can be regenerated by re-running setup:")
    for path, what in leftovers():
        note(f"  {path}")
        note(f"      {what}")
    say()
    ...
    if shutil.which("omarchy-plugin-list") is not None:
        say()
        say("The plugin itself is still installed. To take the icon off the bar:")
